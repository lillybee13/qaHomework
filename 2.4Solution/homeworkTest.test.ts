...
// This is clicking Bernice on left side bar and then displaying her info card.
      );
...
/* This is clearing Bernice's name field, inputting "Test Name" into the name field, 
clicking on Phillips name on the left side bar, and then confirming that Phillip's info card appears.*/
    ...
    /* This is clicking Bernice on the left side bar and displaying her info card 
    which is expecting the name field value to be "Bernice Ortiz". */
    ...
    test("A canceled change doesn't persist", async () => {
          ...
          expect(await driver.findElement(nameInput).getAttribute("value")).toBe("Phillip Weaver")
    });
/*This is clicking Phillip on the left side bar, displaying his info card, removing his name from the name field, entering "Test Name" into the name field, 
clicking cancel, and then verifying that the name field says "Phillip Weaver".*/
    test("A saved change persists", async () => {
          ...
          */
          ...
          await driver.findElement(bernice).click();
          expect(await driver.findElement(nameInput).getAttribute("value")).toBe("Test Name");
/* This is clicking Bernice on the left side bar, displaying her info card, clearing the name field, entering "Test Name" into the name field,
    clicking Save, clicking Phillip on the left side bar, clicking Bernice on the left side bar (which is going to say Test Name now),
    and then verifying that "Test Name" is in the name field. */
    });
  });
...
/* This is clicking Bernice on the left side bar, displaying her info card, clearing the name field, 
    putting a space in the name field and then back spacing,
    clicking save, and making sure the error appears correctly.*/
      });
...
/* This is repeating the test above, then adding a space into the name field, clicking save, 
    and making sure the error does not appear. */
      });
  });
});